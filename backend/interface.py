...
def user_information_by_username(username):
    ...
    if is_email:
        ...
        # profile = UserProfile.objects.create(user=result[0]).userprofile
...
def resource_information_list(course_id, number):
    ...
    result = []
    temp = list(
        Resource.objects.filter(course_code=course_id).values_list('id', 'upload_user_id', 'download_count', 'name',
                                                                   'upload_time'))
    ...
    for item in temp:
        if (item[1] == None):
            ...
            resource = {'resource_id': item[0], 'username': User.objects.get(id=int(item[1])).username,
                        'download_count': item[2], 'name': item[3]}
