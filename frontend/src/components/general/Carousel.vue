        ...
        <el-tabs v-model="active_post_type" class="tab" type="card" @tab-click="handle_tab_clicked">
          <el-tab-pane name="hot_post">
            ...
            <template v-for="thread in hot_threads">
                      ...
                      <el-col :span="4" :offset="1">
                        <el-button type="text" @click="course_name_clicked(thread.course_id)">
                          <el-col :span="16" :offset="1">
                            <p style="color:#58B7FF;padding-top: 5px;" class="course_name">
                              (课程：{{ thread.course_name }}) </p>
                          </el-col>
                        </el-button>
                      </el-col>
              ...
              <center>
                <hr width="90%" style="margin-top: 20px; border: none;border-top: 1px solid rgb(241,242,244)" />
              </center>
            </template>
          </el-tab-pane>
        ...
        $.ajax({
          ...
          success: function (rdata) {
            ...
            $.ajax({
              ...
              type: 'POST',
              data: {
                id: rdata['resource_info']['upload_user_id']
              },
              async: false,
              ...
              $.ajax({
                ...
                success: function (data) {
                  ...
                  for (var i = 0; i < info_list.length; i++) {
                    var type = (info_list[i].category === 1 ? '问题讨论' : (info_list[i].category === 2 ?
                      '学习心得' : '其他'))
                    var cut_description = (info_list[i].intro.length < 100 ? info_list[i].intro :
                      info_list[i].intro.substr(0, 100)) + '....'
                    var thread = {
                      id: id_list[i],
                      agree_num: info_list[i].grade_sum,
                      follow_num: info_list[i].follow_count,
                      read_num: info_list[i].click_count,
                      type: type,
