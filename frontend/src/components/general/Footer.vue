<template>
	<footer id="Footer" class="footer">	
		<div align="center"><hr class="line"></div>
		<div class="footer-main">
			<p class="footer_copyright"> © 2020 YNUCourseHub</p>
			<p class="footer_link_container">
				<a href="https://github.com/YNUSharedHub/YNUSharePlatform.git" target="_blank" class="footer_link">Go to our Github: https://github.com/YNUSharedHub/YNUSharePlatform.git</a>
			</p>
			<p class="footer_qq">如果你认为本网站的一些文件侵犯了您的权益，请向我们发送邮件。我们将会彻底清除这些文件。<br/>
				2756699419@qq.com</p>
		</div>
		</div>
	</footer>
</template>

<script type="text/javascript">
	
</script>

<style type="text/css" scpoed>
	.line{
		border:none;
		border-top: 1px solid #ccc;
		margin-top: 20px;
		width: 90%;
		height: 0px;
	}
	.footer{
		position: absolute;
		font-family: Microsoft Yahei;
		background-size: 100%;
		//background-color: #58B7FF;
		width: 100%;
		height: 100px;
		text-align: center;
		margin-top: 50px;
	}
	.footer_link_container{
		margin-top: 10px;
	}
	.footer_link{
		text-decoration: none;
		margin-right: 25px;
		margin-left: 25px;
		color: #409EFF;
	}
	.footer_copyright{
		padding-top: 20px;
		color:gray;
	}
	.footer_qq{
		color:gray;
		padding-top: 10px;
		padding-bottom: 10px;
	}
</style>
